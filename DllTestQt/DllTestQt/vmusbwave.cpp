...
void VmUsbWave::setdevState(int32_t devstate)
{
    m_devState = devstate;
...
void CALLBACK VmUsbWave::UsbDataReadyCallBack(void* ppara)
{
 ...
 {
     /*int32_t length_ch1 = ReadVoltageDatas(0, buffer_ch1, m_real_length);
     int32_t length_ch2 = ReadVoltageDatas(0, buffer_ch2, m_real_length);
...
QStringList VmUsbWave::getsupportsamples()
{
    ...
    int32_t num = GetOscSupportSampleNum();
    uint32_t* samples = new uint32_t[num];
...
void VmUsbWave::setsample(int32_t s)
{
    m_sample = s;
...
int32_t VmUsbWave::getsample()
{
    return m_sample;
}

int32_t VmUsbWave::getMaxCaptureLength()
{
    return GetMemoryLength();
}

void VmUsbWave::setCaptureLength(int32_t kb)
{
    m_captureLength = kb;
}

int32_t VmUsbWave::getCaptureLength()
{
...
int32_t VmUsbWave::getTriggerMode()
{
    return GetTriggerMode();
}

void VmUsbWave::setTriggerMode(int32_t mode)
{
    SetTriggerMode(mode);
}

static uint32_t trigger_index[]={0x0000, 0x0001, 0x0002, 0x0004, 0x0008, 0x0010, 0x0020, 0x0040, 0x0080, 0x0100};

int32_t VmUsbWave::getTriggerStyle()
{
    uint32_t style = GetTriggerStyle();

    int32_t i=0;
    for(int32_t k=0; k<sizeof(trigger_index)/sizeof(int32_t); k++)
    {
...
void VmUsbWave::setTriggerStyle(int32_t style)
{
    SetTriggerStyle(trigger_index[style]);
}

int32_t VmUsbWave::getTriggerSource()
{
    return GetTriggerSource();
}

void VmUsbWave::setTriggerSource(int32_t sr)
{
    SetTriggerSource(sr);
}

int32_t VmUsbWave::getTriggerLevel()
{
    return GetTriggerLevel();
}

void VmUsbWave::setTriggerLevel(int32_t level)
{
